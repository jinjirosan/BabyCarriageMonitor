...
class weatherdata():
    ...
    def rainchance(pressure):
        ...
        return raintable[max({k: raintable[k] for k in raintable if k < weatherdata.read_enviro_pressure()})]
...
def collect_env_data():  ## dict must contain all the sanitized clean data to transmit

    temp_cpu = sysmon.get_cpu_temperature()
    temp_raw = round(weather.temperature(), 2)
    temp = round(read_enviro_temp(), 2)
    heading = round(weatherdata.read_enviro_heading(), 0)
    axes = read_enviro_accelerometer()
    pressure = round(weatherdata.read_enviro_pressure(), 1)
    rainchancepercentage = weatherdata.rainchance(pressure)
    e_light = weatherdata.read_enviro_light()
    temp_c_headliner = round(weatherdata.read_temp_headliner(), 2)
